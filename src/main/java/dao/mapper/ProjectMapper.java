...
public interface ProjectMapper {
	...
	+ " values(#{dbuser.id}, #{project.category}, #{project.summary}, #{project.story}, #{project.main_imageurl}, #{project.banner_imageurl}, '승인대기', #{project.goal}, #{project.deadline}, #{dbuser.account}, #{project.subject})") 
	...
	@Select("SELECT project.project_num, project.id, project.category ,project.summary, project.banner_image banner_imageurl, project.subject, project.summary, project.main_image main_imageurl, TIMESTAMPDIFF(day,NOW(),deadline) 'deadline', ifnull(support.support_money,0) 'support_money', ifnull(ROUND(100 * SUM(support.support_money) / project.goal),0) pop " + 
			"FROM project Left JOIN support ON project.project_num = support.project_num WHERE project.state != '승인대기' GROUP BY project.project_num")
	List<Project> select();
	...
	@Select("SELECT project.project_num, project.id, project.summary, project.banner_image banner_imageurl, project.subject, project.summary, project.main_image main_imageurl, TIMESTAMPDIFF(day,NOW(),deadline) 'deadline', ifnull(support.support_money,0) 'support_money', ifnull(ROUND(100 * SUM(support.support_money) / project.goal),0) 'pop' " + 
			"FROM project Left JOIN support ON project.project_num = support.project_num WHERE project.state != '승인대기' GROUP BY project.project_num ORDER BY project.project_num desc")
	List<Project> newselect();

	@Select("SELECT project.project_num, project.id, project.summary, project.banner_image banner_imageurl, project.subject, project.summary, project.main_image main_imageurl, TIMESTAMPDIFF(day,NOW(),deadline) 'deadline', ifnull(support.support_money,0) 'support_money', ifnull(ROUND(100 * SUM(support.support_money) / project.goal),0) 'pop' " + 
			"FROM project Left JOIN support ON project.project_num = support.project_num WHERE project.state != '승인대기' GROUP BY project.project_num Having deadline > 0 and deadline < 11 ORDER BY deadline")
	List<Project> endselect();

	@Select("SELECT project.project_num, project.id, project.summary, project.banner_image banner_imageurl, project.subject, project.summary, project.main_image main_imageurl, TIMESTAMPDIFF(day,NOW(),deadline) 'deadline', ifnull(support.support_money,0) 'support_money', ifnull(ROUND(100 * SUM(support.support_money) / project.goal),0) 'pop' " + 
			"FROM project Left JOIN support ON project.project_num = support.project_num WHERE project.state != '승인대기' GROUP BY project.project_num HAVING pop > 150 and deadline > 0 ORDER BY pop DESC")
	...
	@Select("<script>" +
			...
			"	<if test='prostate != null'>and deadline ${prostate} NOW()</if> " + 
			"	<if test='rate != null'>and ${rate}</if> " +
			"	<if test='state != null'>and state != '승인대기'</if>" +
			"</trim>" +
			"</script>")
